...
public class DocumentServiceTest {
    ...
    void testUploadDocumentFileWithFullPipeline() {
        ...
        when(textExtractionService.extractText(any(), eq("PDF"))).thenReturn("Database Management Systems: Normalization and Queries.");
        ...
        DocumentChunk chunk = DocumentChunk.builder().id(10L).chunkIndex(0).content("Database Management Systems: Normalization and Queries.").build();
        ...
        DocumentResponse response = documentService.uploadDocument(userA, mockFile, "DBMS Notes", "DBMS study guide", null);
    ...
    void testGetDocumentChunksSuccess() {
        ...
        DocumentChunk chunk0 = DocumentChunk.builder().id(101L).document(doc).chunkIndex(0).content("Chunk 0 content").build();
        DocumentChunk chunk1 = DocumentChunk.builder().id(102L).document(doc).chunkIndex(1).content("Chunk 1 content").build();

        when(documentRepository.findById(1L)).thenReturn(Optional.of(doc));
        when(documentChunkRepository.findByDocumentIdOrderByChunkIndexAsc(1L)).thenReturn(List.of(chunk0, chunk1));
    ...
    void testGetDocumentChunksUnauthorized() {
        ...
        assertThrows(SecurityException.class, () ->
                documentService.getDocumentChunks(userB, 1L)
        );
    }
    ...
    void testGetDocumentContentUnauthorized() {
        ...
        assertThrows(SecurityException.class, () ->
                documentService.getDocumentContent(userB, 1L)
        );
    }
    ...
    void testGetUserDocuments() {
        Document doc1 = Document.builder()
                .id(1L).user(userA).fileName("DBMS Notes.pdf").title("DBMS Notes").fileType("PDF").fileSize(1843200L).status(DocumentStatus.READY)
                .build();
        Document doc2 = Document.builder()
                .id(2L).user(userA).fileName("Project Architecture.docx").title("Project Architecture").fileType("DOCX").fileSize(850000L).status(DocumentStatus.READY)
                .build();

        when(documentRepository.findByUserIdOrderByCreatedAtDesc(userA.getId())).thenReturn(List.of(doc1, doc2));
    ...
    void testGetDocumentFileSuccess() {
        Document doc = Document.builder()
                .id(1L).user(userA).fileName("DBMS Notes.pdf").fileUrl("user_101/stored_dbms.pdf").status(DocumentStatus.READY)
                .build();
